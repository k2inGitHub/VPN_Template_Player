...
- (void)addList {
    ...
    for(int i=0; i<5; i++){
        ...
        fileModel.name = name;
        fileModel.fileCreationDate = @"ad";
        [localFileArray addObject:fileModel];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([file.name isEqualToString:@"girl0"]){
        cell.textLabel.text=@"丝袜诱惑";
        cell.detailTextLabel.text=[NSString stringWithFormat:@"%ld MB        %@",(long)1250,@"2016-01-15 23:20"];
    }
    if ([file.name isEqualToString:@"girl1"]){
        cell.textLabel.text=@"未上锁的房间";
        cell.detailTextLabel.text=[NSString stringWithFormat:@"%ld MB        %@",(long)3560,@"2016-01-15 23:21"];
    }
    if ([file.name isEqualToString:@"girl2"]){
        cell.textLabel.text=@"小电影";
        cell.detailTextLabel.text=[NSString stringWithFormat:@"%ld MB        %@",(long)15240,@"2016-01-15 23:22"];
    }
    if ([file.name isEqualToString:@"girl3"]){
        cell.textLabel.text=@"情侣游戏";
        cell.detailTextLabel.text=[NSString stringWithFormat:@"%ld MB        %@",(long)685,@"2016-01-15 23:23"];
    }
    if ([file.name isEqualToString:@"girl4"]){
        cell.textLabel.text=@"mm社区";
        cell.detailTextLabel.text=[NSString stringWithFormat:@"%ld MB        %@",(long)53547,@"2016-01-15 23:24"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([file.fileCreationDate isEqualToString:@"ad"]){
